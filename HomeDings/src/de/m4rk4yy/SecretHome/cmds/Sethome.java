...
public class Sethome implements CommandExecutor{
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if(p.hasPermission("cmi.command.sethome.10") && !p.isOp() && !p.hasPermission("*")) {
			...
			if(countOfHomes >= 10) {
				p.sendMessage(mm.getErrorPrefix() + "Maximale Anzahl von zehn Homes erreicht.");
				return true;
			}
		}
		...
		if(p.hasPermission("cmi.command.sethome.8") && !p.isOp() && !p.hasPermission("*")) {
			int countOfHomes = mm.homeCount(p.getUniqueId());
			if(countOfHomes >= 8) {
				p.sendMessage(mm.getErrorPrefix() + "Maximale Anzahl von acht Homes erreicht.");
				return true;
			}
		}
		...
		if(p.hasPermission("cmi.command.sethome.2") && !p.isOp() && !p.hasPermission("*")) {
			int countOfHomes = mm.homeCount(p.getUniqueId());
			if(countOfHomes >= 2) {
				p.sendMessage(mm.getErrorPrefix() + "Maximale Anzahl von zwei Homes erreicht.");
				return true;
			}
		}
		...
		if(args.length == 1 && !args[0].contains("-")) {
			...
			if(mm.nameExists(p.getUniqueId(), homename)) {
				p.sendMessage(mm.getErrorPrefix() + "Das Home " + homename + " existiert bereits.");
				return true;
			}
